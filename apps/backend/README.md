...
```bash
kubectl -n data-platform apply -f infra/k8s/addons/teradata-mock-postgres.yaml
```

2. backend 환경값 설정(예시):

```bash
kubectl -n data-platform patch configmap platform-config --type merge -p '{
  "data": {
...
```bash
kubectl -n data-platform patch secret platform-secrets --type merge -p '{
  "stringData": {
...
```bash
kubectl -n data-platform rollout restart deployment/backend
```
